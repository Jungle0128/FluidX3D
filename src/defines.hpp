...
#define GRAPHICS_FRAME_HEIGHT 1920 // set frame height if only GRAPHICS is enabled
#define GRAPHICS_BACKGROUND_COLOR 0x87CEEB // set background color; black background (default) = 0x000000, white background = 0xFFFFFF
#define GRAPHICS_U_MAX 0.24f // maximum velocity for velocity coloring in units of LBM lattice speed of sound (c=1/sqrt(3)) (default: 0.18f)
// 调整速度颜色映射范围，使其对应物理风速
// u_max = 0.24 lu 对应 15 m/s（高风速危险区域）
...
#define GRAPHICS_Q_CRITERION 0.0005f // Q-criterion value for Q-criterion isosurface visualization (default: 0.0001f)
#define GRAPHICS_STREAMLINE_SPARSE 16 // set how many streamlines there are every x lattice points,默认为每8个格点一个流线。数字越大，流线越少
...
// ========== 监控区域参数 ==========
#define MONITOR_X_MIN 0.25f
#define MONITOR_X_MAX 0.65f
#define MONITOR_Y_MIN 0.40f
#define MONITOR_Y_MAX 0.55f
#define MONITOR_Z_MIN_CELLS 1u    // 最小 Z（格子数，uint 类型）
#define MONITOR_Z_MAX 0.6f        // 占 Nz 的比例
#define DANGER_VELOCITY 0.16f     // 危险速度阈值 (lu)，对应 10 m/s